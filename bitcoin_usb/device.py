...
from typing import *
from .address_types import AddressType, AddressTypes, SignerInfo, get_hwi_address_type, public_descriptor_info
...
class Device:    
    ...
    def __enter__(self):
        self.lock.acquire()
        self.client = hwi_commands.get_client(device_type=self.enumerate_device['type'], 
    ...
    def get_xpubs(self)->Dict[AddressTypes, str]:
        ...
        for s in vars(AddressTypes):
            ...
            xpubs[address_type] = self.client.get_pubkey_at_path(address_type.key_origin(self.network)).to_string()
        return xpubs
    ...
    def display_address(self,  descriptor_str:str, keychain:bdk.KeychainKind,  address_index:int,  network:bdk.Network)->str:
        desc_infos = public_descriptor_info(descriptor_str)
        


        if desc_infos.address_type.is_multisig:
            pubkey_providers = [signer_info.to_hwi_pubkey_provider() for signer_info in desc_infos.signer_infos]
            return self.client.display_multisig_address( get_hwi_address_type(desc_infos.address_type), HWIMultisigDescriptor(pubkey_providers, )
                                                        
                                                        )
        else:
            return self.client.display_singlesig_address(desc_infos.address_type.get_bip32_path(network,keychain, address_index), get_hwi_address_type(desc_infos.address_type))
