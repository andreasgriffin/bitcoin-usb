...
from hwilib.descriptor import parse_descriptor, Descriptor, PubkeyProvider, WPKHDescriptor, WSHDescriptor, PKHDescriptor, MultisigDescriptor, TRDescriptor, SHDescriptor
...
def get_hwi_address_type(address_type:AddressType)->HWIAddressType:
    ...
    if address_type.name  in [AddressTypes.p2sh_p2wpkh.name, AddressTypes.p2sh_p2wsh.name ]:
...
class SignerInfo():
    ...
    def from_hwi(pubkey_provider: PubkeyProvider)-> "SignerInfo":
        return SignerInfo(xpub = pubkey_provider.pubkey,
    ...
    def to_hwi_pubkey_provider(self)-> PubkeyProvider:
        ...
        provider = PubkeyProvider(origin= hwi_key_origin, 
               pubkey=self.xpub,
...
def _find_matching_address_type(instance_tuple, address_types:List[AddressType]):
    for address_type in address_types:
        if len(instance_tuple) == len(address_type.hwi_descriptor_classes) and all( isinstance(i, c)    for i,c in zip(instance_tuple, address_type.hwi_descriptor_classes)):
            return address_type
    return None


class DescriptorInfo:
    def __init__(self, address_type:AddressType, signer_infos:List[SignerInfo], threshold=1) -> None:
...
def public_descriptor_info(descriptor_str: str) -> DescriptorInfo:
    ...
    address_type = _find_matching_address_type(_get_descriptor_instances(hwi_descriptor), get_address_types())        
    if not address_type:
    ...
    return DescriptorInfo(address_type=address_type,signer_infos=[SignerInfo.from_hwi(pubkey_provider) for pubkey_provider in pubkey_providers] ,threshold= threshold )  
...
def make_multisig_descriptor(
    ...
    assert all([descriptor.network == descriptors[0].network for descriptor in descriptors]), "Descriptors with different networks given"
    
    hwi_pubkey_providers = []
    for descriptor in descriptors:
        hwi_pubkey_provider = _descriptor_to_hwi_pubkey_providers(descriptor.as_string_private())
    ...
    multisig_hwi_descriptor =   address_type.hwi_descriptor_classes(pubkeys=hwi_pubkey_providers, thresh=threshold, is_sorted=True)
    
    return bdk.Descriptor(multisig_hwi_descriptor.to_string(), network=descriptors[0].network)
